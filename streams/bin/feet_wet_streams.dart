...
main() {
  ...
  singleWhere();
  singleError();
  singleErrorWithCatchError();
  singleErrorWithCatchAsync();
...
Future singleStreamAsync() async {
  // BEGIN(consuming_a_stream_async)
  // BEGIN(simple_stream_code_async)
  var data = [1, 2, 3, 4, 5]; // some sample data
  var stream = new Stream.fromIterable(data); // create the stream
  // END(simple_stream_code_async)
...
singleWhere() async {
  ...
  result = await broadcastStream.singleWhere((value) => value < 2);
...
singleError() {
  ...
  Future inner() async {
    ...
    try {
      ...
      result = await broadcastStream.single;
...
void readingAFile() {
  ...
  var thisFile = Platform.script;

  File file = new File(thisFile.toFilePath());
  ...
  file
      ...
      .listen((String data) => print(data), // output the data
          ...
          onDone: () => print('Finished reading a file')
          );

  // END(string_decoder)
}

Future readingAFileAsync() async {
  // Read this script file
  var thisFile = Platform.script;

  File file = new File(thisFile.toFilePath());
  // BEGIN(string_decoder_async)
  // BEGIN(reading_a_file_async)
  try {
    await for (var data in file.openRead().transform(UTF8.decoder)) {
      print('ReadingAFileAsync:');
      print(data);
    } 
    print('Finished reading a file (with await for)');
  } catch (e) {
      print('Error, could not open file');
  }
  // END(string_decoder_async)
}
// END(reading_a_file_async)
