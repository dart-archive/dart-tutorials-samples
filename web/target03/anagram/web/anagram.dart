...
void main() {
  ...
  clearButton.on.click.add(newletters);
  
  generateNewLetters();
}

void moveToWord(Event e) {
  Element letter = e.target;
  result.children.add(letter);
  wordvalue += scrabbleValues[letter.text];
  value.text = "$wordvalue";
  letter.on.click.remove(moveToWord);
  letter.on.click.add(moveToLetters);
}

void moveToLetters(Event e) {
  Element letter = e.target;
  letterpile.children.add(letter);
  wordvalue -= scrabbleValues[letter.text];
  value.text = "$wordvalue";
  letter.on.click.remove(moveToLetters);
  letter.on.click.add(moveToWord);
}
...
generateNewLetters() {
  ...
  for (var i = 0; i < 7; i++) {
    ...
    buttons[i].on.click.add(moveToWord);
