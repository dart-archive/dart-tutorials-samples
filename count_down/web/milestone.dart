...
class Milestone extends Observable {
  ...
  Map toRaw() {
    return {'milestoneName': milestoneName, 'happensOn': happensOn.toString(),};
  }
...
class MilestoneStore {
  ...
  void _initializeDatabase(idb.VersionChangeEvent e) {
    ...
    var objectStore =
        db.createObjectStore(mileStoneStore, autoIncrement: true);

    // Create an index to search by name,
    // unique is true: the index doesn't allow duplicate milestone names.
    objectStore.createIndex(nameIndex, 'milestoneName', unique: true);
  }
  ...
  Future _loadFromDB(idb.Database db) {
    ...
    var trans = db.transaction(mileStoneStore, 'readonly');
    var store = trans.objectStore(mileStoneStore);
  ...
  Future<Milestone> add(String milestoneName, DateTime occursOn) {
    ...
    var transaction = _db.transaction(mileStoneStore, 'readwrite');
    var objectStore = transaction.objectStore(mileStoneStore);
  ...
  Future remove(Milestone milestone) {
    ...
    var transaction = _db.transaction(mileStoneStore, 'readwrite');
    transaction.objectStore(mileStoneStore).delete(milestone.dbKey);
  ...
  Future clear() {
    // Clear database.
    var transaction = _db.transaction(mileStoneStore, 'readwrite');
    transaction.objectStore(mileStoneStore).clear();
